...
export function MobileApp({bundleId, setBundleId, setGoogleServicesJson, setGoogleServiceInfoPlist}: Props) {
    ...
    const onGoogleJsonRefChanges = (file: File | null) => {
        ...
        actionPostFile(file)
            .then((resp) => {setGoogleServicesJson(resp.data.results[0].location)})
    }
    ...
    const onPlistFileChange = (file: File | null) => {
        ...
        actionPostFile(file)
            .then((resp) => {setGoogleServiceInfoPlist(resp.data.results[0].location)})
    }
        ...
        <div className="settings-mobile">
            ...
            <div className="form-controls">
                ...
                <input 
                    ...
                    accept=".json"
                    className="hidden" 
                    onChange={(e) => onGoogleJsonRefChanges(e.target.files && e.target.files[0])}
                />
                <button 
                ...
                <input 
                    ...
                    accept=".plist"
                    className="hidden" 
                    onChange={(e) => onPlistFileChange(e.target.files && e.target.files[0])}
                />
                <button onClick={() => plistFileRef.current?.click()} className="gen-secondary-btn mb-16">Upload</button>
