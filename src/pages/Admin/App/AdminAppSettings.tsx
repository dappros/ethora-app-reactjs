...
export function AdminAppSettings() {
    ...
    // signon tab
    const [enableEmail, setEnableEmail] = useState(app.signonOptions.includes('email'))
    const [enableGoogle, setEnableGoogle] = useState(app.signonOptions.includes('google'))
    const [enableApple, setEnableApple] = useState(app.signonOptions.includes('apple'))
    const [enableFacebook, setEnableFacebook] = useState(app.signonOptions.includes('facebook'))
    const [enableMetamask, setEnableMetamask] = useState(app.signonOptions.includes('metamask'))

    // web app
    const [domainName, setDomainName] = useState(app.domainName)
    const [firebaseWebConfigString, setFirebaseWebConfigString] = useState(app.firebaseWebConfigString ? app.firebaseWebConfigString : '')

    // mobile app
    const [bundleId, setBundleId] = useState(app.bundleId)
    const [googleServicesJson, setGoogleServicesJson] = useState(app.googleServicesJson)
    const [googleServiceInfoPlist, setGoogleServiceInfoPlist] = useState(app.googleServiceInfoPlist)

    const [availableMenuItems, setAvailableMenuItems] = useState(app.availableMenuItems)
    ...
    // visibility 
    const [defaultAccessAssetsOpen, setDefaultAccessAssetsOpen] = useState(app.defaultAccessAssetsOpen)
    const [defaultAccessProfileOpen, setDefaultAccessProfileOpen] = useState(app.defaultAccessProfileOpen)
