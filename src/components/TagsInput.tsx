...
export function TagsInput({ value, onChange, suggestions = [], placeholder, autoFocus }: Props) {
  ...
  return (
    <div className="relative">
      <div className="flex flex-wrap items-center gap-2 rounded-xl bg-[#F5F7F9] px-3 py-2 min-h-[44px]">
        {value.map((tag) => (
        ...
        <div className="absolute z-10 mt-1 w-full rounded-xl bg-white shadow-lg border border-gray-100 py-1">
          {matches.map((m) => (
            <button
              ...
              onClick={() => add(m)}
              className="block w-full text-left px-3 py-1.5 text-sm hover:bg-[#F5F7F9]"
            >
              {m}
            </button>
          ))}
