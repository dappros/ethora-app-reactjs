...
export function RadioButton({ value, label, className = '' }: Props) {
  return (
    <Field className={`grid grid-cols-[16px,_1fr] items-center cursor-pointer ${className}`}>
      <Radio
        value={value}
      ...
      {label && <Label className="cursor-pointer inline-block text-sm ml-2">{label}</Label>}
    </Field>
  );
}
